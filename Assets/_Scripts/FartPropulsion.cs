...
public class FartPropulsion : MonoBehaviour
{
    ...
    public void OnJumpButtonDown(BaseEventData eventData)
    {
        if (!isGameOver)
        {
            ...
            // Always play the sound when button is pressed
            PlayRocketSound();
        }
    }
    ...
    void ApplyContinuousFartForce()
    {
        if (fartPower > 0)
        {
            ...
            // Always play the rocket sound when applying force
            PlayRocketSound();
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Obstacle") || IsGrounded())
        {
            ...
            // Only stop the rocket sound when grounded, not for obstacle collisions
            if (IsGrounded())
            {
                StopRocketSound();
                isFlying = false; // Reset flying state only when grounded
            }
        }
    }
